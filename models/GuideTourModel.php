...
class GuideTourModel
{
    ...
    public function getThisWeekTours($guide_id)
    {
        ...
        WHERE s.guide_id = :guide_id
        AND YEARWEEK(s.start_date, 1) = YEARWEEK(:today, 1)
    ...
    public function getTodayTourDayNumber($start_date)
    {
        ...
        $now = new DateTime(today()); // Lấy ngày hiện tại
    ...
    {
        // 1. Lấy danh sách khách hàng thuộc Schedule này (từ schedules -> bookings -> booking_customers)
        $sqlCustomers = "
            SELECT 
        ...
        // 2. Lấy tất cả trạng thái điểm danh cho các khách trong tour này
        $sqlAttendance = "
            SELECT 
                customer_id, 
                activity_id, 
        ...
        $rawAttendanceData = $stmtAttendance->fetchAll(PDO::FETCH_ASSOC);

        $attendanceData = [];
        // Nhóm dữ liệu điểm danh theo Customer ID -> Activity ID
        foreach ($rawAttendanceData as $record) {
            $c_id = $record['customer_id'];
            $a_id = $record['activity_id'];

            if (!isset($attendanceData[$c_id])) {
                $attendanceData[$c_id] = [];
            }
            // Lưu trạng thái và ghi chú theo activity_id
            $attendanceData[$c_id][$a_id] = [
        ...
        foreach ($customers as $customer) {
            ...
            // Gán trạng thái điểm danh đã nhóm (nếu có)
            $customer['attendance'] = $attendanceData[$customer_id] ?? [];
    ...
    {
        // Dùng REPLACE INTO thay vì INSERT ... ON DUPLICATE (đơn giản & an toàn hơn khi có AUTO_INCREMENT)
        $sql = "
        REPLACE INTO attendance_activity 
    ...
    ";
        ...
        if ($result) {
            $this->saveOrUpdateAttendance($schedule_id, $customer_id, $status);
        }

        return $result;
    }
    public function saveOrUpdateAttendance($schedule_id, $customer_id, $status = 'absent')
    {
        // Bảng 'attendance' KHÔNG có khóa UNIQUE/Primary Key trên (schedule_id, customer_id)
        // Nên phải dùng logic kiểm tra TỒN TẠI thủ công.

        $sqlCheck = "SELECT id FROM attendance WHERE schedule_id = :sid AND customer_id = :cid LIMIT 1";
        $stmtCheck = $this->conn->prepare($sqlCheck);
        $stmtCheck->execute([':sid' => $schedule_id, ':cid' => $customer_id]);
        $existingId = $stmtCheck->fetchColumn();

        if ($existingId) {
            // UPDATE status nếu bản ghi đã tồn tại
            $sqlUpdate = "UPDATE attendance SET status = :status, checked_at = NOW() WHERE id = :id";
            $stmtUpdate = $this->conn->prepare($sqlUpdate);
            return $stmtUpdate->execute([':status' => $status, ':id' => $existingId]);
        } else {
            // INSERT nếu chưa tồn tại
            $sqlInsert = "INSERT INTO attendance (schedule_id, customer_id, status) VALUES (:sid, :cid, :status)";
            $stmtInsert = $this->conn->prepare($sqlInsert);
            return $stmtInsert->execute([':sid' => $schedule_id, ':cid' => $customer_id, ':status' => $status]);
        }
    }
    ...
    public function getTourStatsThisWeek($guide_id)
    {
        ...
        $sql = "
            ...
            FROM schedules
            WHERE guide_id = :guide_id 
            AND YEARWEEK(start_date, 1) = YEARWEEK(:today, 1)
        ";
