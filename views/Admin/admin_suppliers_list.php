...
<div class="max-w-[1800px] mx-auto p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center space-x-3">
            ...
            <button class="flex items-center gap-2 px-4 py-2 border rounded-md text-sm text-dark hover:bg-main hover:text-white transition-all duration-200">
                Lọc
            </button>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-6 py-4 text-center relative flex justify-end group">
                        ...
                        <div class="absolute right-0 top-0 mt-2 w-40 bg-white border border-slate-200 rounded-md shadow-lg text-sm py-1 
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full p-6">
            ...
            <form action="?mode=admin&act=add-supplier" method="POST">
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div><label class="block text-sm font-medium mb-1">Người liên hệ</label><input type="text" name="contact_name" class="w-full border rounded px-3 py-2 mt-1"></div>
                    <div><label class="block text-sm font-medium mb-1">Email</label><input type="email" name="contact_email" class="w-full border rounded px-3 py-2 mt-1"></div>
                    <div><label class="block text-sm font-medium mb-1">Số điện thoại</label><input type="text" name="contact_phone" class="w-full border rounded px-3 py-2 mt-1"></div>
                    <div><label class="block text-sm font-medium mb-1">Địa chỉ</label><input type="text" name="address" class="w-full border rounded px-3 py-2 mt-1"></div>
                </div>
                <div class="mb-4">
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full p-6">
            ...
            <form action="?mode=admin&act=update-supplier" method="POST">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-medium mb-1">Tên nhà cung cấp *</label>
                        <input type="text" name="name" value="<?= htmlspecialchars($editSupplier['supplier_name'] ?? '') ?>" required class="w-full border rounded px-3 py-2 mt-1">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Loại dịch vụ *</label>
                        <select name="supplier_types_id" required class="w-full border rounded px-3 py-2 mt-1">
                    ...
                    <div>
                        ...
                        <input type="text" name="contact_name" value="<?= htmlspecialchars($editSupplier['contact_name'] ?? '') ?>" class="w-full border rounded px-3 py-2 mt-1">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Email</label>
                        <input type="email" name="contact_email" value="<?= htmlspecialchars($editSupplier['contact_email'] ?? '') ?>" class="w-full border rounded px-3 py-2 mt-1">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Số điện thoại</label>
                        <input type="text" name="contact_phone" value="<?= htmlspecialchars($editSupplier['contact_phone'] ?? '') ?>" class="w-full border rounded px-3 py-2 mt-1">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Địa chỉ</label>
                        <input type="text" name="address" value="<?= htmlspecialchars($editSupplier['address'] ?? '') ?>" class="w-full border rounded px-3 py-2 mt-1">
                    </div>
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-medium mb-1">Mô tả</label>
                    <textarea name="description" rows="3" class="w-full border rounded px-3 py-2 mt-1"><?= htmlspecialchars($editSupplier['description'] ?? '') ?></textarea>
                </div>
