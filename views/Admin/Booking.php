...
<div class="max-w-[1900px] mx-auto p-6">
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-medium text-slate-500 uppercase pr-6">hành động</th>
                    </tr>
                </thead>
                    ...
                    <?php else: foreach ($bookings as $b): ?>
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-4 py-4">
                                ...
                                $statusText = $status == 'cho_xac_nhan' ? 'chờ xác nhận' :
                                             ($status == 'da_coc' ? 'đã cọc' :
                                             ($status == 'hoan_tat' ? 'hoàn tất' : $status));
                                $statusColor = $status == 'hoan_tat' ? 'bg-green-100 text-green-800' :
                                              ($status == 'da_coc' ? 'bg-blue-100 text-blue-800' :
                            ...
                            <td class="px-4 py-4 text-right pr-6 space-x-3">
                                <a href="?mode=admin&act=booking&view_id=<?= $b['id'] ?>" 
                                   class="text-indigo-600 hover:text-indigo-900 font-medium">xem</a>
                                <a href="?mode=admin&act=booking&edit_id=<?= $b['id'] ?>" 
    ...
    <div class="mt-6 flex gap-4 text-xs">
        <span class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-yellow-100"></div> chờ xác nhận</span>
        <span class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-blue-100"></div> đã cọc</span>
        <span class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-green-100"></div> hoàn tất</span>
        <span class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-purple-100"></div> đoàn</span>
        <span class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-blue-100"></div> lẻ</span>
    </div>
</div>